...
const DocumentEditor = () => {
  const { editorState, setEditorState, editorRef,handleEditorChange, currentFont,focusEditor  } =
  ...
  const addPageBreak = () => {
    const contentState = editorState.getCurrentContent();
    const contentStateWithPageBreak = contentState.createEntity('PAGE_BREAK', 'IMMUTABLE', { style: { width: '100%', height: '1px', backgroundColor: 'black', display: 'block', marginBottom: '29cm' } });
    const entityKey = contentStateWithPageBreak.getLastCreatedEntityKey();
  ...
  return (
    <div
      ...
      <div className="absolute top-[200px] right-[100px] w-[400px] flex flex-col gap-5 p-8 rounded-lg shadow-lg bg-white">
        ...
        <button onClick={addPageBreak} className="btn bg-blue-500 text-white py-2 px-4 rounded-md hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
          Add Page Break
        </button>
      </div>

      <div className="absolute top-[200px] left-[100px] w-[400px] flex flex-col gap-5 p-8 rounded-lg shadow-lg white-blur-glass items-center">
        <p className="text-gray-600">Click on Mic to speak.</p>
        <div className="p-3 border border-black rounded-full shadow-2xl blue-blur-glass">
        < FaMicrophone onClick={handleGenerateText} className="w-[80px] h-[80px] text-white">
          {loading ? 'Loading...' : 'Generate Response'}
        </FaMicrophone>
        </div>
      </div>
